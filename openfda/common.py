...
class BatchHelper(object):
    ...
    def __exit__(self, type, value, traceback):
        ...
        logging.info(
            "BatchHelper: %d operations in %.2f seconds, %.2f docs/s",
            self._count,
            elapsed,
            self._count / elapsed,
        )
...
def strip_unicode(raw_str, remove_crlf=False):
    ...
    a_str = raw_str
    if remove_crlf:
        a_str = a_str.replace("\x13", "").replace("\x0A", "")
...
def first_file_timestamp(path):
    ...
    """
    return (
        arrow.get(os.path.getmtime(os.path.join(path, os.listdir(path)[0]))).format(
            "YYYY-MM-DD"
        )
    ...
    return (
        arrow.get(
            sorted(
                list(
                    map(
                        lambda file: os.path.getmtime(os.path.join(path, file)),
                        os.listdir(path),
                    )
                )
            )[-1]
