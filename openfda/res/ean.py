...
codeLength = {"EAN13": 13, "EAN8": 8, "UPCA": 12, "UPCE": 8}


def __len_check(chk, _type="EAN13"):
    return (len(chk) == codeLength[_type]) or (len(chk) == codeLength[_type] - 1)
...
def ean_check_digit(chk, code="EAN13"):
    ...
    if chk.isdigit() and __len_check(chk):
        ...
        t = (
            ...
            - (
                (
                    __sum_digits(chk, start=0, end=_len, mult=m0)
                    + __sum_digits(chk, start=1, end=_len, mult=m1)
                )
                % 10
            )
            % 10
...
def ean_13_valid(chk):
    ...
    return chk.isdigit() and __len_check(chk) and (int(chk[-1]) == ean_check_digit(chk))
...
def upce2a(chk):
    ...
    if chk.isdigit() and __len_check(chk, "UPCE"):
        ...
        if t is not None:
            ...
            elif len(chk) == codeLength["UPCE"] and int(chk[-1]) == upca_check_digit(t):
                t = t + chk[-1]
            else:
                t = None
    return t
