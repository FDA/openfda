...
class JoinMapper(Mapper):
    ...
    def map_shard(self, map_input, map_output):
        ...
        assert self.table in PLUCK_MAP, "Could not find %s in PLUCK_MAP" % self.table
        ...
        for val in value:
            ...
            if self.table == "registration":
                ...
                for product in products:
                    ...
                    if new_key:
                        output.add(
                            new_key, (self.table, pluck(self.pluck_list, new_value))
                        )
            else:
...
class DeviceAnnotateMapper(parallel.Mapper):
    ...
    def map_shard(self, map_input, map_output):
        ...
        if self.table == "device_clearance":
            self.table = "510k"
...
class DeviceHarmonization(luigi.Task):
    ...
    def requires(self):
        ...
        return [
            classification_pipeline.Classification2JSON(),
...
class Harmonized2OpenFDA(luigi.Task):
    ...
    def run(self):
        mapreduce(
            Collection.from_sharded(self.input().path),
