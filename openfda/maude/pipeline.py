...
DEVICE_PROBLEM_CODES_FILE = join(
    BASE_DIR, "maude/extracted/events/deviceproblemcodes.txt"
)
PATIENT_PROBLEM_CODES_FILE = join(
    BASE_DIR, "maude/extracted/events/patientproblemdata.txt"
)
...
FILE_HEADERS = {
    "foidevproblem": ["mdr_report_key", "problem_code"],
    "patientproblemcode": [
        "mdr_report_key",
...
def _fix_date(input_date):
    ...
    try:
        ...
        if input_date:
            logging.info(
                "unparseable date: %s with input %s", input_date, formated_date
            )
...
class ExtractAndCleanDownloadsMaude(luigi.Task):
    ...
    def run(self):
        ...
        for i in range(len(self.input())):
            ...
            download_util.extract_and_clean(
                input_dir, "ISO-8859-1//TRANSLIT", "UTF-8", "txt"
            )
...
class PreprocessFilesToFixIssues(AlwaysRunTask):
    ...
    def _run(self):
        for filename in glob.glob(self.input().path + "/*/*foi*.txt") + glob.glob(
            self.input().path + "/*/device*.txt"
        ):
...
class CSV2JSONMapper(parallel.Mapper):
    ...
    def handle_oversized_foitext(self, value):
        ...
        value[no_columns - 1] = (
            combined_text[:-1] if combined_text.endswith("|") else combined_text
        )
    ...
    def map(self, key, value, output):
        ...
        if file_type == "foidev":
            if len(value) not in [28, 45]:
                logging.info(
                    "Does not conform to foidev structure. Skipping: %s, %s",
                    mdr_key,
                    "#" * 5,
                )
                output.add(
                    file_type, "%s: missing fields" % mdr_key + ":" + "|".join(value)
                )
                return
        elif file_type == "mdrfoi":
            if len(value) not in [77, 81]:
                logging.info(
                    "Does not conform to mdrfoi structure. Skipping: %s, %s",
                    mdr_key,
                    "#" * 5,
                )
                output.add(
            ...
            logging.info(
                "Does not conform to %s structure. Skipping: %s, %s",
                file_type,
                mdr_key,
                "#" * 5,
            )
        ...
        if file_type == "foidevproblem":
            product_problem = self.device_problem_codes_ref.get(
                new_value["problem_code"]
            )
            new_value["product_problem"] = product_problem

        # Same applies to patient problem codes.
        if file_type == "patientproblemcode":
            patient_problem = self.patient_problem_codes_ref.get(
                new_value["problem_code"]
            )
    ...
    def _join(self, key, values):
        ...
        for patient_problem in val.get("patientproblemcode", []):
            if (
                ...
                patient = {
                    "patient_sequence_number": patient_problem[
                        "patient_sequence_number"
                    ]
                }
            ...
            for patient_problem in val.get("patientproblemcode", []):
                if (
                    ...
                    patient["patient_problems"] = (
                        ...
                        if patient.get("patient_problems") is None
                        else patient["patient_problems"]
                        + [patient_problem["patient_problem"]]
                    )
...
class CSV2JSON(luigi.Task):
    ...
    def run(self):
        ...
        if self.loader_task == "init":
            ...
            input_files = (
                [f for f in files if self.loader_task in f]
                + device_problems
                + patient_problems
            )
        ...
        reader = csv.reader(
            open(DEVICE_PROBLEM_CODES_FILE), quoting=csv.QUOTE_NONE, delimiter="|"
        )
        ...
        reader = csv.reader(
            open(PATIENT_PROBLEM_CODES_FILE), quoting=csv.QUOTE_NONE, delimiter="|"
        )
        for idx, line in enumerate(reader):
            if len(line) > 1:
        ...
        parallel.mapreduce(
            parallel.Collection.from_glob(
                input_files,
                parallel.CSVSplitLineInput(quoting=csv.QUOTE_NONE, delimiter="|"),
            ),
...
class MaudeAnnotationMapper(DeviceAnnotateMapper):
    def filter(self, data, lookup=None):
        ...
        if harmonized:
            ...
            new_reg = [
                d for d in registration if d.get("registration_number") == lookup
            ]
