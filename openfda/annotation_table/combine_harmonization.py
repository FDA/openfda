        ...
        for pivot in record_value:
            ...
            if join_key in new_data_dict:
                for new_data_value in new_data_dict[join_key]:
                    joined_dict = dict(
                        list(pivot.items()) + list(new_data_value.items())
                    )
                ...
                if new_data_key == None:
                    continue
                joined_dict = dict(
                    list(pivot.items()) + list({new_data_key: []}.items())
                )
                record_list.append(joined_dict)
    return record_list


def combine(product_file, spl_extract, rx_extract, unii_extract, upc_extract, json_out):
    ...
    for spl_data in label:
        ...
        clean_label["original_packager_product_ndc"] = spl_data[
            "OriginalPackagerProductNDSs"
        ]
    ...
    record_list = _combine_dicts(joinable_record_dict, joinable_unii, "unii_indexing")
    # Adding RXNorm
    joinable_record_dict = _joinable_dict(record_list, ["spl_set_id", "spl_version"])
