...
class DrugsFDAPipelineTests(unittest.TestCase):
  def setUp(self):
    ...
    openfda.drugsfda.pipeline.RAW_DATA_FILE = join(self.test_dir, 'raw/drugsatfda.zip')
    openfda.drugsfda.pipeline.PRODUCTS_DB = join(self.test_dir, 'json/products.db')
    openfda.drugsfda.pipeline.APPLICATIONS_DB = join(self.test_dir, 'json/applications.db')
    openfda.drugsfda.pipeline.APPLICATIONS_DOCS_DB = join(self.test_dir, 'json/applicationsdocs.db')
    openfda.drugsfda.pipeline.SUBMISSIONS_DB = join(self.test_dir, 'json/submissions.db')
    openfda.drugsfda.pipeline.SUBMISSION_PROPERTY_TYPE_DB = join(self.test_dir, 'json/submissionpropertytype.db')
    openfda.drugsfda.pipeline.MARKETING_STATUS_DB = join(self.test_dir, 'json/marketingstatus.db')
    ...
    openfda.drugsfda.pipeline.ANNOTATED_DB = join(self.test_dir, 'json/annotated.db')
    print('Temp dir is ' + self.test_dir)

    os.mkdir(dirname(os.path.abspath(openfda.drugsfda.pipeline.RAW_DATA_FILE)))
    shutil.copyfile(os.path.join(dirname(os.path.abspath(__file__)), "drugsatfda.zip"),
                    openfda.drugsfda.pipeline.RAW_DATA_FILE)

    zip_filename = os.path.join(dirname(os.path.abspath(__file__)), "harmonized.json.zip")
  ...
  def test_annotated_json(self):
    ...
    ldb = leveldb.LevelDB(os.path.join(AnnotateDrugsFDA().output().path, 'shard-00000-of-00001.db'))
