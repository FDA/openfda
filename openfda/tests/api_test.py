...
def test_pagination():
  ...
  ndc_data = fetch_headers(ENDPOINT + '/drug/ndc.json?search=marketing_start_date:"20060707"')
  data_link = ndc_data['Link']
  assert data_link == '<' + ENDPOINT + '/drug/ndc.json?search=marketing_start_date%3A%2220060707%22&skip=1&limit=1>; rel="next"'
  ...
  assert event_data['Link'] == '<' + ENDPOINT + '/drug/event.json?skip=200&limit=100>; rel="next"'
