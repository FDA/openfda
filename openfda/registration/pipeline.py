...
class DownloadRegListing(luigi.Task):
    ...
    def run(self):
        common.download(
            self.url, os.path.join(self.local_dir, "registration_listing.txt")
        )
...
def remap_supplemental_files(original, supplemental, output_file):
    ...
    combined["premarket_submission_number"] = combined[
        "premarket_submission_number"
    ].astype("str")
        ...
        def transform_dates(coll):
            ...
            def _replace_date(k, v):
                ...
                if v is None:
                    return (k, v)
                if k in DATE_KEYS:
                    return (k, arrow.get(v, "MM/DD/YYYY").format("YYYY-MM-DD"))
...
class RegistrationJoinReducer(parallel.Reducer):
    def _join(self, values):
        ...
        if agent_data:
            us_agent = dict(
                list(agent_data[0].items()) + list(us_agent_address.items())
            )
...
class JoinEstablishmentTypes(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(
            config.data_dir("registration/establishment_listing.db")
        )
...
class JoinListings(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(
            config.data_dir("registration/registration_listing.db")
        )
