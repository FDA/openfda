...
def refresh_index(index_name):
    ...
    resp = requests.post(
        "http://%s/%s/_refresh" % (config.es_host(), index_name), timeout=100000
    )
...
def optimize_index(index_name, wait_for_merge=1):
    ...
    resp = requests.post(
        "http://%s/%s/_forcemerge?max_num_segments=1" % (config.es_host(), index_name),
        timeout=100000,
    )
    resp.raise_for_status()
...
def dump_index(
    es, index, endpoint, target_dir, cleaner=None, query=None, chunks=100000
):
    """
    ...
    return_dict = {
        "meta": {
            ...
            "results": {"skip": 0, "limit": chunks, "total": total_docs},
        },
        "results": [],
    }
    ...
    for result in elasticsearch.helpers.scan(
        ...
        if docs_written % chunks == 0:
            ...
            logging.info("Writing %20s, %.2f%% done", index, 100.0 * total_written)
...
def index_without_checksum(es, index, doc_type, batch):
    ...
    create_batch = []
    for doc in batch:
        create_batch.append({"_index": index, "_type": doc_type, "_source": doc})

    create_count, errors = elasticsearch.helpers.bulk(
        es, create_batch, raise_on_error=False, raise_on_exception=False
    )
    for item in errors:
    ...
    if len(errors) > 0:
        logging.warning(
            "%s create batch complete: %s created %s errors",
            index,
            create_count,
            len(errors),
        )
...
def index_with_checksum(es, index, doc_type, batch):
    ...
    existing_docs = {}
    found_docs = es.transport.perform_request(
        "GET",
    ...
    for docid, checksum, doc in batch_with_checksum:
        ...
        if es_checksum == checksum:
            ...
            logging.debug(
                "Checksum mismatch for %s, [ %s vs %s ]", docid, checksum, es_checksum
            )
    ...
    index_count, errors = elasticsearch.helpers.bulk(
        es, index_batch, raise_on_error=False, raise_on_exception=False
    )
    ...
    logging.info(
        "%s update batch complete: %s total, %s indexed, %s skipped, %s errors",
        index,
        len(index_batch) + skipped,
        len(index_batch),
        skipped,
        len(errors),
    )
...
class ResetElasticSearch(AlwaysRunTask):
    ...
    def _run(self):
        logging.info(
            "Create index and loading mapping: %s/%s",
        ...
        if self.delete_index:
            elasticsearch_requests.clear_and_load(
                es,
                self.target_index_name,
                self.target_type_name,
...
class LoadJSONMapper(parallel.Mapper):
    ...
    def _map_incremental(self, map_input):
        ...
        with BatchHelper(
            index_with_checksum, es, self.index_name, self.type_name
        ) as helper:
            for key, doc in map_input:
                assert (
                    self.docid_key in doc
                ), "Document is missing id field: %s" % json.dumps(doc, indent=2)
    ...
    def _map_non_incremental(self, map_input):
        es = elasticsearch.Elasticsearch(self.es_host, timeout=120)
        with BatchHelper(
            index_without_checksum, es, self.index_name, self.type_name
        ) as helper:
            for key, doc in map_input:
                helper.add(doc)
    ...
    def _run(self):
        ...
        elasticsearch_requests.update_process_datetime(
            ...
            last_update_date=(
                ...
                if callable(self.last_update_date)
                else self.last_update_date
            ),
        )
