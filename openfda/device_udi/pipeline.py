...
class ExtractXML(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(config.data_dir("device_udi/extracted"), BATCH)
        )
                ...
                for identifier, idenList in iter(xml["identifiers"].items()):
                    if type(idenList) == type([]):
                        for iden in idenList:
                            if (
                                "deviceIdType" in iden
                                and "deviceIdIssuingAgency" in iden
                                and "Primary" == iden["deviceIdType"]
                            ):
                                device["@id"] = (
                                    (
                                        ""
                                        if "deviceIdIssuingAgency" not in iden
                                        else iden["deviceIdIssuingAgency"] + "_"
                                    )
                                    + iden["deviceId"]
                                ).lower()
...
class XML2JSON(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(config.data_dir("device_udi/json.db"), BATCH)
        )
...
class AnnotateDevice(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(config.data_dir("device_udi/annotate.db"), BATCH)
        )
