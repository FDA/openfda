...
def extract_id(tree):
    return first_match_or_empty_string(tree.getroot().xpath("/document/id/@root"))
...
def extract_original_packager_product_ndcs(tree):
    ...
    ndc_xpath = '//definingMaterialKind/code[@codeSystem="%s"]/@code' % NDC_OID
    return tree.getroot().xpath(ndc_xpath)


def extract_package_ndcs(tree):
    """Extracts the full labelercode-productcode-packagecode NDCs"""
    ndc_xpath = '//containerPackagedProduct/code[@codeSystem="%s"]/@code' % NDC_OID
    return tree.getroot().xpath(ndc_xpath)
