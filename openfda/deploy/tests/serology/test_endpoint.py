...
def test_date_ranges():
  assert_total(
    '/device/covid19serology.json?search=manufacturer.exact:Euroimmun+AND+device.exact:"SARS-COV-2 ELISA (IgG)"', 110)
  assert_total(
    '/device/covid19serology.json?search=manufacturer:Euroimmun+AND+device:"SARS-COV-2 ELISA (IgG)"', 110)
  assert_total(
    '/device/covid19serology.json?search=manufacturer:Euroimmun+AND+device:"SARS-COV-2"', 110)
...
def test_sample_no_ranges():
  ...
  assert_total('/device/covid19serology.json?search=sample_no:[1+TO+15]&limit=100', 60)
