...
def test_record_with_multiple_packaging():
  meta, results = fetch(
    '/drug/ndc.json?search=product_ndc:46122-062')
  eq_(len(results), 1)
  ...
  eq_(packaging[0]["description"], "50 TABLET, FILM COATED, EXTENDED RELEASE in 1 BOTTLE (46122-062-71)")
  eq_(packaging[0]["marketing_start_date"], "20020430")
  eq_(packaging[0]["sample"], False)

  eq_(packaging[1]["package_ndc"], "46122-062-78")
  eq_(packaging[1]["description"], "100 TABLET, FILM COATED, EXTENDED RELEASE in 1 BOTTLE (46122-062-78)")
...
def test_exact_fields():
  assert_total('/drug/ndc.json?search=openfda.brand_name.exact:Amoxicillin and Clavulanate Potassium', 1)
...
def test_drug_with_most_number_of_packaging():
  meta, results = fetch(
    '/drug/ndc.json?search=product_ndc:66083-741')
  eq_(len(results), 1)
