...
def test_counts():
  ...
  assert (next(x for x in status if x['endpoint'] == 'devicepma'))['documents'] >= 33000
  assert (next(x for x in status if x['endpoint'] == 'deviceudi'))['documents'] >= 2480000
  assert (next(x for x in status if x['endpoint'] == 'drugevent'))['documents'] >= 11512000
  assert (next(x for x in status if x['endpoint'] == 'druglabel'))['documents'] >= 160000
  assert (next(x for x in status if x['endpoint'] == 'foodevent'))['documents'] >= 83000
