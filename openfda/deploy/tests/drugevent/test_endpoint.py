...
def test_drug_event_unii():
  assert_unii('/drug/event.json?search=safetyreportid:10003301', ['RM1CE97Z4N', 'WK2XYI10QM'])

def test_drug_generic_name_count():
  assert_total('/drug/event.json?search=patient.drug.medicinalproduct:TOVIAZ', 7700)
  assert_total('/drug/event.json?search=patient.drug.openfda.generic_name:%22dimethyl%20fumarate%22', 90000)

def test_histogram():
  meta, counts = fetch_counts('/drug/event.json?search=receivedate:[20040101+TO+20180611]&count=receivedate')
