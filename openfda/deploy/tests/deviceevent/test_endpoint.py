...
def test_multiple_patient_problems():
  meta, results = fetch(
    '/device/event.json?search=mdr_report_key:4102973')
  eq_(len(results), 1)
  event = results[0]
...
def test_multiple_patients_with_same_problem():
  meta, results = fetch(
    '/device/event.json?search=mdr_report_key:4718251')
  eq_(len(results), 1)
  ...
  for idx, patient in enumerate(sorted(event["patient"], key=lambda k: k["patient_sequence_number"])):
    ...
    eq_(sorted(patient['patient_problems'], key=lambda k: k), ["Injury", "Tissue Damage"])
