...
def test_size_value_as_string_fda82():
  meta, results = fetch(
    '/device/udi.json?search=identifiers.id:10801741065450')
  eq_(len(results), 1)
...
def test_booleans():
  ...
  meta, results = fetch(
    '/device/udi.json?search=is_direct_marking_exempt:true')
  ok_(meta["results"]["total"] >= 51 + 10777 + 13555 + 2447 + 17088 + 3955 + 3383 + 6036 + 5168 + 2265 + 913)
...
def test_date_fields():
  ...
  udi = results[0]
  ok_(udi["identifiers"][0]["id"] in ["50699753498278",
                                      "00699753498273",
                                      "00850490007061",
                                      "00850490007054",
                                      "00850490007085",
...
def test_openfda():
  meta, results = fetch(
    '/device/udi.json?search=identifiers.id:00085412096094')
  eq_(len(results), 1)
  ...
  eq_(udi["product_codes"][0]["openfda"]["device_name"].lower(), "agent, absorbable hemostatic, collagen based")
  eq_(udi["product_codes"][0]["openfda"]["medical_specialty_description"], "General, Plastic Surgery")
...
def test_storage():
  meta, results = fetch(
    '/device/udi.json?search=identifiers.id:00887517614384')
  eq_(len(results), 1)
  ...
  # KEEP DRY
  meta, results = fetch(
    '/device/udi.json?search=identifiers.id:00886874103951')
  eq_(len(results), 1)
...
def test_catalog_number():
  meta, results = fetch(
    '/device/udi.json?search=catalog_number:521554BE2')
  eq_(len(results), 1)
  ...
  meta, results = fetch(
    '/device/udi.json?search=catalog_number.exact:14-521554BE2')
  eq_(len(results), 1)
...
def test_device_sizes():
  meta, results = fetch(
    '/device/udi.json?search=identifiers.id:00886874104361')
  eq_(len(results), 1)
  ...
  meta, results = fetch(
    '/device/udi.json?search=identifiers.id:00888480590095')
  eq_(len(results), 1)
...
def test_CoRoent():
  ...
  eq_(udi["gmdn_terms"][0]["definition"],
  ...
  eq_(udi["product_codes"][0]["name"], "Intervertebral fusion device with bone graft, lumbar")
  eq_(udi["product_codes"][0]["openfda"]["device_class"], "2")
  eq_(udi["product_codes"][0]["openfda"]["device_name"], "Intervertebral Fusion Device With Bone Graft, Lumbar")
  eq_(udi["product_codes"][0]["openfda"]["medical_specialty_description"], "Orthopedic")
  ...
  eq_(udi["product_codes"][1]["openfda"]["device_name"], "Spinal Vertebral Body Replacement Device")
  eq_(udi["product_codes"][1]["openfda"]["medical_specialty_description"], "Orthopedic")
  ...
  eq_(udi["sterilization"]["sterilization_methods"], "Moist Heat or Steam Sterilization")
...
def test_product_code_count():
  assert_count_top('/device/udi.json?count=product_codes.code.exact',
                   ['MNI', 'MNH', 'KWP', 'KWQ'])


def test_mri_safety_count():
  assert_count_top('/device/udi.json?count=mri_safety.exact',
                   ['Labeling does not contain MRI Safety Information', 'MR Conditional', 'MR Unsafe', 'MR Safe'])
