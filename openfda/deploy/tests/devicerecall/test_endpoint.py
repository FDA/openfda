...
def test_pma_parsed_out():
  meta, results = fetch('/device/recall.json?search=product_res_number.exact:Z-0567-2013')
  eq_(sorted(results[0]['pma_numbers']), ['P960009S007', 'P960009S051', 'P960009S134'])


def test_510k_parsed_out():
  meta, results = fetch('/device/recall.json?search=product_res_number.exact:Z-1535-2009')
  eq_(sorted(results[0]['k_numbers']), ['K003205', 'K984357', 'K990848', 'K991723'])


def test_product_code_ooo():
  meta, results = fetch('/device/recall.json?search=product_res_number.exact:Z-2153-2014')
  eq_(results[0]['product_code'], 'N/A')


def test_product_code_na():
  meta, results = fetch('/device/recall.json?search=product_res_number.exact:Z-0094-2021')
  eq_(results[0]['product_code'], 'N/A')


def test_old_product_code():
  meta, results = fetch('/device/recall.json?search=product_res_number.exact:Z-0351-03+AND+res_event_number:25150')
...
def test_other_submission_description():
  ...
  meta, results = fetch('/device/recall.json?search=product_res_number.exact:Z-1650-2010')
  eq_(results[0]['other_submission_description'], 'Illegally Marketed')


def test_unparseable_address_flattened():
  meta, results = fetch('/device/recall.json?search=product_res_number.exact:Z-0432-2021')
  eq_(results[0]['recalling_firm'], 'Inpeco S.A. Via San Gottardo 10 Lugano Switzerland')
...
def test_one_complete_record():
  ...
  eq_(results[0]['product_description'],
  ...
  eq_(results[0]['distribution_pattern'],
  ...
  eq_(results[0]['reason_for_recall'],
...
def test_exact_count_0():
  assert_count_top('/device/recall.json?count=root_cause_description.exact',
                   ['Device Design', 'Nonconforming Material/Component', 'Process control', 'Software design',
                    'Other'])
...
def test_date_range_2():
  assert_total('/device/recall.json?search=event_date_terminated:([20050922+TO+20051231])', 600)
