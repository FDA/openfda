...
class MROutput(object):
    ...
    class Writer(object):
        ...
        def put(self, key, value):
            assert (
                False
            ), "Don't use this class directly: use an output like LevelDBOutput"
    ...
    def create_writer(self, prefix, shard_idx, num_shards):
        ...
        assert shard_idx < num_shards, "Invalid shard index (%d > %d)" % (
            shard_idx,
            num_shards,
        )
...
class JSONOutput(MROutput):
    ...
    def finalize(self, tmp_dir, final_dir):
        ...
        assert len(files) == 1, (
            "JSONOutput expected one temporary file, got: %s" % files
        )
        logger.info(
            "Moving temporary file: %s to final destination: %s", files[0], final_dir
        )
...
class JSONLineOutput(MROutput):
    ...
    def finalize(self, tmp_dir, final_dir):
        ...
        subprocess.check_output(
            'cat %s/*.jsonline > "%s"' % (tmp_dir, final_dir), shell=True
        )
    ...
    class Writer(MROutput.Writer):
        ...
        def put(self, key, value):
            json_str = json.dumps(
                convert_unicode(value), ensure_ascii=False, encoding="utf-8"
            )
