...
def _run_reducer(reducer, queue, tmp_prefix, output_format, output_tmp, i, num_shards):
    with watchdog.Watchdog():
        try:
            ...
            reducer.initialize(
                queue, tmp_prefix, output_format, output_tmp, i, num_shards
            )
            reducer.shuffle()
...
def mapreduce(
    ...
    managers = [
        multiprocessing.Manager() for i in range(int(max(map_workers, num_shards)))
    ]
    shuffle_queues = [
        managers[i % len(managers)].Queue(1000) for i in range(int(num_shards))
    ]
    ...
    logger.info(
        "Starting MapReduce: %s -> %s@%s, M: %s, R: %s",
        inputs,
        output_prefix,
        num_shards,
    ...
    tmp_prefix = output_prefix + "-mapreduce-tmp-%s" % arrow.utcnow().format(
        "YYYY-MM-DD-hh-mm"
    )
    output_tmp = output_prefix + "-mapreduce-output-%s" % arrow.utcnow().format(
        "YYYY-MM-DD-hh-mm"
    )
    ...
    try:
            ...
            for split in input_collection:
                mapper_tasks.append(
                    mapper_pool.apply_async(
                        _run_mapper, args=(mapper, shuffle_queues, split)
                    )
                )
        ...
        while len(map_results) < len(mapper_tasks):
            for i, task in enumerate(mapper_tasks):
                ...
                if isinstance(result, WrappedException):
                    ...
                    raise MRException(
                        "Mapper %d failed." % i, child_message=result.exception_message
                    )
                # else:
        ...
        for i, task in enumerate(reducer_tasks):
            ...
            if isinstance(result, WrappedException):
                ...
                raise MRException(
                    "Reducer %d failed." % i, child_message=result.exception_message
                )
