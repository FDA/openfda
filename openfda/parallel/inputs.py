    ...
    def compute_splits(self, filename, desired_splits):
        ...
        with open(filename, "rb") as f:
            while f.tell() < file_len:
                f.seek(split_size, 1)
                f.readline()
                splits.append(FileSplit(filename, self, last_pos, f.tell()))
                last_pos = f.tell()
...
class CSVSplitLineInput(MRInput):
    ...
    def create_reader(self, split):
        return CSVSplitLineInput.Reader(
            split, delimiter=self.delimiter, quoting=self.quoting
        )
    ...
    def compute_splits(self, filename, desired_splits):
        ...
        split_size = max(
            ...
            int(
                file_len
                / (desired_splits if self.fixed_splits is None else self.fixed_splits)
            ),
        )
        splits = []
        last_pos = 0
            ...
            while f.tell() < file_len:
                ...
                splits.append(
                    FileSplit(
                        ...
                        self,
                        last_pos,
                        f.tell() if f.tell() < file_len else file_len,
                    )
                )
                last_pos = f.tell()
        ...
        def entries(self):
            ...
            while f.tell() < self.end_pos:
                ...
                if not line:
                    f.close()
                    return
                yield str(f.tell()), csv.reader(
                    io.StringIO(line), delimiter=self.delimiter, quoting=self.quoting
                ).__next__()
    ...
    class Reader(MRInput.Reader):
        def entries(self):
            for idx, line in enumerate(
                open(self.filename, encoding="utf-8", errors="ignore")
            ):
...
class CSVDictLineInput(MRInput):
    ...
    def create_reader(self, split):
        return CSVDictLineInput.Reader(
            split,
